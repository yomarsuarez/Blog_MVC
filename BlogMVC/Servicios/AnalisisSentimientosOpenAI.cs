...
namespace BlogMVC.Servicios
{
    public class AnalisisSentimientosOpenAI : IAnalisisSentimientos
    {
        ...
        public AnalisisSentimientosOpenAI(ApplicationDbContext context, IOptions<ConfiguracionesIA> options, HttpClient httpClient)
        {
            this.context = context;
            this.options = options;
            httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", options.Value.LlaveOpenAI);
        ...
        public async Task AnalizarComentariosPendientes()
        {
            ...
            var inputField = await SubirArchivoPeticiones(jsonContent);
            var batchId = await EnviarBatch(inputField);
        ...
        private async Task<string> SubirArchivoPeticiones(string jsonContent)
        {
            ...
            HttpResponseMessage respuestaSubidaArchivo = await httpClient.PostAsync("https://api.openai.com/v1/files", form);

            string responseContent = await respuestaSubidaArchivo.Content.ReadAsStringAsync();

            var inputField = JsonDocument.Parse(responseContent)
        ...
        private async Task<string> EnviarBatch(string inputField)
        {
            var configuracionesIA = options.Value;

            var batchRequest = new
            {
                input_file_id = inputField,
        ...
        private async Task GuardarLoteYActualizarComentarios(List<Comentario> comentariosPendienteDeAnalisis, string batchId)
        {
            var lote = new Lote
            {
                Id = batchId,
            ...
            var idsComentarios = comentariosPendienteDeAnalisis.Select(x => x.Id);
        ...
        public async Task ProcesarLotesPendientes()
        {
            var lotesSinProcesar = await context.Lotes
                .Where(x => x.Estatus != "failes" && x.Estatus != "completed" && x.Estatus != "expired").ToListAsync();
            ...
            foreach(var lote in lotesSinProcesar)
            {
                ...
                if(status == "completed")
                {
                    ...
                    var respuestaContenidoArchivoString = await respuestaContenidoArchivo.Content.ReadAsStringAsync();

                    var lineas = respuestaContenidoArchivoString.Split("\n", StringSplitOptions.RemoveEmptyEntries);
