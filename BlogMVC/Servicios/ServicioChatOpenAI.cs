...
namespace BlogMVC.Servicios
{
    public class ServicioChatOpenAI : IServicioChat
    {
        ...
        private string systemPromptGenerarCuerpo = """            
            ...
            Te esfuerzas para que los principiantes entiendas las cosas dando ejemplos prácticos.
            """;

        private string ObtenerPromptGenerarCuerpo(string titulo) => $"""
            Crear un articulo para un blog. El título del artículo será {titulo}.

            Si lo entiendes conveniente, debes insertar tips.

            El formato de respuesta es HTML. Por tanto, debes colocar negritas donde consideres, títulos, subtítulos, entre 
            otras cosas que ayuden a resaltar el formato.

            La respuesta no debe ser un documento HTML, si no solamente el artículo en formato HTML, 
        ...
        public async Task<string> GenerarCuerpo(string titulo)
        {
            ...
            var prompUsuario = ObtenerPromptGenerarCuerpo(titulo);

            var mensajeUsuario = new UserChatMessage(prompUsuario);
        ...
        public async IAsyncEnumerable<string> GenerarCuerpoString (string titulo)
        {
            var modeloTexto = options.Value.ModeloTexto;
