...
namespace BlogMVC.Controllers
{
    public class EntradasController : Controller
    {
        ...
        public EntradasController(ApplicationDbContext context, IAlmacenadorArchivos almacenadorArchivos,
            IServicioUsuarios servicioUsuarios, IServicioChat servicioChat, IWebHostEnvironment env, IServicioImagenes servicioImagenes)
        {
        ...
        public async Task<IActionResult> Crear(EntradaCrearViewModel modelo)
        {
            ...
            {
                var archivo = Base64AFormFile(modelo.ImagenPortadaIA);
        ...
        {
            var entrada = await context.Entradas.IgnoreQueryFilters().FirstOrDefaultAsync(x => x.Id == id);
        ...
        public async Task GenerarCuerpo([FromQuery] string titulo)
        {
            ...
            await foreach (var segmento in servicioChat.GenerarCuerpoString(titulo))
            {
        ...
        [HttpGet]
        public async Task<IActionResult> ConComentariosNegativos()
        {
            var entradasConComentariosNegativos = await context.Entradas
                ...
                .Select(x => new EntradaConComentariosNegativosViewModel
                {
                    Id = x.Id,
                    Titulo = x.Titulo,
                    CantidadComentariosNegativos = x.Comentarios.Where(x => x.Puntuacion == 2 || x.Puntuacion == 1).Count()
                }).ToListAsync();
        ...
        private IFormFile Base64AFormFile (string base64)
        {
